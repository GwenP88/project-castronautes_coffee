...
async function runOverlaySequence() {
    ...
    // 1) Dots "analyse en cours" pendant 3s
    await animateDots(nightDots, 3000);
    nightDots.textContent = '.';

    // 2) Frappe ligne par ligne
    const lines = script
        .split('\n')
...
function activateNightMode() {
    document.documentElement.setAttribute('data-theme', 'night'); // ← ajout
    document.body.setAttribute('data-theme', 'night');
    document.body.classList.add('is-night-mode');
    localStorage.setItem('theme', 'night');

    document.querySelectorAll('.is-day').forEach(function(el) {
...
function deactivateNightMode() {
    document.documentElement.removeAttribute('data-theme'); // ← ajout
